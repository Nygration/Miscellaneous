def hycom_hindcast2(tlat,tlon,startstr,stopstr,tdep=0):
    import numpy as np
    import datetime
    import urllib
    import warnings
...
#     depurl = 'http://tds.hycom.org/thredds/dodsC/GLBy0.08/expt_93.0.ascii?depth%5B0:1:39%5D'
    ...
    startdt = datetime.datetime.fromisoformat(startstr)
    stopdt = datetime.datetime.fromisoformat(stopstr)
    starttime = (startdt - datetime.datetime(2000,1,1,0,0,0)).total_seconds()/3600 # match units for the HYCOM API
    stoptime = (stopdt - datetime.datetime(2000,1,1,0,0,0)).total_seconds()/3600 # match units for the HYCOM API
    tdiff = (stopdt-startdt).total_seconds()/3600 # now in hours
    ...
    timestartidx = [idx for idx,val in enumerate(np.abs(timelist - starttime)) if val == np.min(np.abs(timelist-starttime))][0]
    timestopidx = [idx for idx,val in enumerate(np.abs(timelist - stoptime)) if val == np.min(np.abs(timelist-stoptime))][0]
    timeidx=f"{timestartidx}:1:{timestopidx}"
    if tdep==0:
    ...
    rrr0 = urllib.request.Request(uurl)
    rrr1 = urllib.request.urlopen(rrr0)
    data = (rrr1.read())
    data = data.decode()
    ...
    print(f"Results are of shape {np.shape(currentlist[0])}")
    return currentlist